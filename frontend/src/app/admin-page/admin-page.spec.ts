...
describe('AdminPage visibility safety', () => {
  ...
  const product = {
    ...
    nutritionLabelStatus: null as string | null,
    servingSizeGrams: 43,
  };
  ...
  it('pages the product list and keeps the page after an edit', () => {
    api.products.mockImplementation((...args: unknown[]) =>
      of({ items: [product] as unknown[], total: 1234, page: (args[5] as number) ?? 1, pageSize: 50 }),
    );

    page.onDataFilterChange('missingNutrition', true);
    expect(api.products).toHaveBeenLastCalledWith('', false, true, false, false, 1);
    expect(page.productPageCount()).toBe(25);

    page.goToProductPage(3);
    expect(api.products).toHaveBeenLastCalledWith('', false, true, false, false, 3);
    ...
    expect(api.products).toHaveBeenLastCalledWith('', false, true, false, false, 25);
    ...
    expect(api.products).toHaveBeenLastCalledWith('', false, true, true, false, 1);
  });
  ...
  it('asks the API for the needs-manual list on its own, without a search', () => {
    ...
    expect(api.products).toHaveBeenLastCalledWith('', false, false, false, true, 1);
  ...
  it('falls back to the last page when the current one emptied', () => {
    api.products.mockImplementation((...args: unknown[]) => {
      const requested = (args[5] as number) ?? 1;
    ...
    expect(api.products).toHaveBeenLastCalledWith('', false, true, false, false, 2);
